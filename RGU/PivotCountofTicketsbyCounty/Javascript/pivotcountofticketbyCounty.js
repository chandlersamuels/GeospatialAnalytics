...
Header = "<b>Shentel: Total tickets by county</b>"
Header2 = "Monthly"
...
d3.queue() //used to ensure that all data is loaded into the program before execution
  ...
  .defer(d3.csv, "Data/ticketcountbyCounty.csv", function(d) {
    if(isNaN(d.tickets)){
      ...
      pivotcountData.set(d.state + '-' + d.county, +d.tickets)//sets the key as the id plus converts string to int
      pivotcountDataArray.push(+d.tickets)
    }
  })
  .await(ready);
}
...
function ready(error, data){//ready function starts the program once all data is loaded
  ...
  d3.select("svg.pivotCount").selectAll("path") //assign the projected map to the svg in HTML
    ...
    .on("mouseover", function(d) {
      ...
      var state = pivotcountData.get(d.tickets)
    	div.transition()
      	   .duration(200)
           .style("opacity", .9)
